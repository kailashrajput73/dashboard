...
export default function AdminProductGroups() {
  ...
  const load = useCallback(async () => {
    try {
      const [nextGroups, nextProducts] = await Promise.all([listProductGroups(), listCatalog()]);
      setGroups(nextGroups || []);
      setProducts(nextProducts || []);
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "Failed to load product groups");
    }
  }, []);
  ...
  function productsFor(group: ProductGroup) {
    ...
    return products.filter((item) => ids.has(item.id) || (item.productGroup || "").toLowerCase() === group.name.toLowerCase());
  }
    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      ...
      {loading ? <View style={styles.center}><ActivityIndicator size="large" color={colors.primary} /></View> : (
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>No product groups yet.</Text>}
          renderItem={({ item }) => {
            const open = openId === item.id;
            const listed = productsFor(item);
            return (
              <View style={styles.card} testID={`product-group-row-${item.id}`}>
      ...
      <AppModal testID="product-group-editor" visible={editor !== undefined} onClose={() => setEditor(undefined)} title={editor ? "Edit product group" : "New product group"}>
        ...
        {filteredProducts.map((product) => (
          <TouchableOpacity key={product.id} testID={`group-product-${product.id}`} style={styles.product} onPress={() => toggleProduct(product.id)}>
            <View style={[styles.checkbox, selected.includes(product.id) && styles.checked]}>{selected.includes(product.id) && <Ionicons name="checkmark" size={15} color="#FFFFFF" />}</View>
            <View style={styles.main}>
              <Text style={styles.productName}>{product.name}</Text>
              <Text style={styles.count}>{product.productCode || "Legacy product"}</Text>
            </View>
          </TouchableOpacity>
        ))}
...
const styles = StyleSheet.create({
  ...
  selection: { color: colors.textSecondary, fontSize: 12, marginBottom: spacing.sm },
  product: { flexDirection: "row", alignItems: "center", gap: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.border, paddingVertical: spacing.sm },
  checkbox: { width: 22, height: 22, borderRadius: 5, borderWidth: 1, borderColor: colors.borderStrong, alignItems: "center", justifyContent: "center" },
  checked: { backgroundColor: colors.primary, borderColor: colors.primary },
  productName: { color: colors.textPrimary, fontWeight: "600" },
});
