...
export default function AdminPurchases() {
  ...
  async function exportPurchasesCsv() {
    ...
    const header = "purchaseId,createdAt,productCode,productName,quantity,listPrice,purchaseDiscount";
    const rows = purchases.flatMap((tx) =>
      tx.lines.map((line) =>
        [tx.id, tx.createdAt, line.productCode, line.productName, line.quantity, line.listPrice, line.purchaseDiscount ?? 0].map(cell).join(","),
      ),
    );
    try {
      await Linking.openURL(`data:text/csv;charset=utf-8,${encodeURIComponent([header, ...rows].join("\n"))}`);
    } catch {
